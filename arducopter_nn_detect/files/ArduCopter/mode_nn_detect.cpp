 ...
 * the detector pulls samples from the file instead of the IMU. Two replay
 * styles are supported:
 *
 *   - Bench replay (default):   the airframe state is ignored, the
 *     hover-stability gate is skipped and inference starts immediately
 *     after init().
 *   - Hover-gated replay:       set NN_DETECT_CSV_HOVER=1 — the detector
 *     behaves like the live-IMU path (arm + takeoff + stable hover for 3 s)
 *     and only then starts consuming the CSV. Used to validate the entire
 *     flight stack with realistic vibration data on top.
 *
 * NN_DETECT_CSV_LOOP=1 loops the file (useful for long-haul checks); by
...
bool ModeNNDetect::init(bool ignore_checks)
{
    ...
    if (_csv_replay_active && !_csv_require_hover) {
        // Bench replay: data is fed from disk, airframe state is ignored.
...
void ModeNNDetect::run()
{
    ...
    // except when we are bench-replaying a CSV (no hover requirement), where
    // the data source is independent of the airframe state.
    const bool gated_replay = _csv_replay_active && _csv_require_hover;
    const bool bench_replay = _csv_replay_active && !_csv_require_hover;
    if (!bench_replay && (!motors->armed() || copter.ap.land_complete)) {
...
void ModeNNDetect::csv_replay_open()
{
    ...
    _csv_replay_loop = (loop_env != nullptr && loop_env[0] != '\0' &&
                       loop_env[0] != '0');
    const char *hover_env = getenv("NN_DETECT_CSV_HOVER");
    _csv_require_hover = (hover_env != nullptr && hover_env[0] != '\0' &&
                         hover_env[0] != '0');
    gcs().send_text(MAV_SEVERITY_INFO,
                    "NN_DETECT: CSV replay %s%s%s",
                    path,
                    _csv_replay_loop ? " loop" : "",
                    _csv_require_hover ? " hover-gated" : "");
#endif
}
