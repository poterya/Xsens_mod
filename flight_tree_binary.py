#!/usr/bin/env python3
"""Flight test detector: NN v2.0 binary tree only.

Console contract during flight: print only "ПОЛОМКА" when the binary tree
network reports fault. Everything else is saved into flight_test/<flight>/.
"""
...
def _resolve_model(base: Path, explicit: Path | None, candidates: tuple[str, ...]) -> Path:
    ...
    for name in candidates:
        ...
        if path.is_file():
            return path.resolve()
    raise SystemExit("Binary tree model not found")
...
def _save_decision_plot(out_dir: Path, decision_rows, smooth_rows) -> None:
    ...
    if decision_rows:
        ...
        ax1.plot(t, p_fault, color="tab:purple", linewidth=1.2, label="NN tree p_fault")
    ...
    ax1.set_title("Binary tree decisions")
...
def _save_artifacts(
    ...
    (out_dir / "README.txt").write_text(
        "\n".join(
            [
                "Detector: NN v2.0 binary tree only.",
                "Console prints only the word 'ПОЛОМКА' when the raw NN prediction is fault.",
...
def run_live(base: Path, model_bundle: dict, port: str, baudrate: int) -> Path:
    ...
    out_dir = base / OUTPUT_ROOT_NAME / f"flight_tree_binary_live_{ts}"
...
def run_offline(base: Path, model_bundle: dict, csv_path: Path) -> Path:
    ...
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = base / OUTPUT_ROOT_NAME / f"flight_tree_binary_offline_{ts}"
    out_dir.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Flight test: NN v2.0 binary tree only. Console prints only ПОЛОМКА."
    )
    ...
    model_path = _resolve_model(base, args.model, DEFAULT_TREE_BINARY_CANDIDATES)
