#!/usr/bin/env python3
"""Run NN v2.0 binary detectors (forest + tree) together with Kalman.
...
Artifacts are saved to nn_v2_binary_detection_logs/nn_v2_binary_<live|offline>_<ts>/.
...
OUTPUT_ROOT_NAME = "nn_v2_binary_detection_logs"
...
def _plot(
    ...
    if not (kalman_log or forest_bin_log or tree_bin_log):
        return
    ...
    ax1.set_title("NN v2.0 binary (forest + tree) + Kalman")

    if len(kt):
        ax2.step(kt, (kp >= 0.5).astype(float), where="post", color="tab:blue", linewidth=1.4, label="Kalman FAULT")
    if len(fbin_st):
        ax2.step(fbin_st, fbin_sidx, where="post", color="tab:red", linewidth=1.0, label="forest binary")
    if len(tbin_st):
        ax2.step(tbin_st, tbin_sidx, where="post", color="tab:purple", linewidth=1.0, label="tree binary")
...
def _hint(forest_bin_smooth, tree_bin_smooth, kalman_log) -> str:
    def top(log):
    ...
    return (
        ...
        f"tree binary «{top(tree_bin_smooth)}»; Калман: {kal}"
    )
...
def _maybe_export_and_metrics(
    ...
    decision = _ask_quality_and_position(
        default_hint=_hint(forest_bin_smooth_log, tree_bin_smooth_log, kalman_log),
...
def _run_common_offline(
    ...
    for row_idx, (_, row) in enumerate(df.iterrows()):
        ...
        f_bin, t_bin = _process_window(feats, forest_bin_bundle, tree_bin_bundle)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="NN v2.0 binary forest + tree + Kalman; "
